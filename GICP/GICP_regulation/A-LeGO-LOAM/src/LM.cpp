...
/*
   ...
   //*/ 
    ...
    pcl::PointCloud<pcl::PointXYZ> aligned;
    fast_gicp_speed->align(aligned,initial_guess);
...
/*	
  ...
  //*/  
	...
	// Set number of threads for multi-threading
	fast_gicp_speed->setNumThreads(1); // Adjust number of threads as needed
	fast_gicp_speed->setMaxCorrespondenceDistance(10); // Set maximum correspondence distance
	fast_gicp_speed->setMaximumIterations(50); // Set maximum iterations


	// Vector to store indices of valid points (optional, can be used for reference)
	//std::vector<int> indices;

	// Remove NaN and Inf points from the input point cloud
	//pcl::removeNaNFromPointCloud(*source, *source, indices);

	//std::vector<int> indices_1;
	//pcl::removeNaNFromPointCloud(*target, *target, indices_1);

	// Set input source and target clouds
	fast_gicp_speed->setInputSource(cloud_in_boxed_translate_to_near_mapboxed);
	fast_gicp_speed->setInputTarget(map_final_boxed);

	// Align the source cloud to the target cloud
	//pcl::PointCloud<pcl::PointXYZ> aligned(new pcl::PointCloud<pcl::PointXYZ>());
	pcl::PointCloud<pcl::PointXYZ> aligned;
	fast_gicp_speed->align(aligned, ndt.getFinalTransformation());
	...
	bool converged = fast_gicp_speed->hasConverged();
        ...
        std::cout << "fast_gicp_speed Transformation matrix:\n" << fast_gicp_speed->getFinalTransformation() << std::endl;
