...
void runGICP(pcl::PointCloud<pcl::PointXYZ>::Ptr source,
             ...
             Eigen::Matrix4f initial_guess = Eigen::Matrix4f::Identity(),
             Eigen::Matrix3f R_delta = Eigen::Matrix3f::Zero() ) {
...
/*
   ...
   //*/ 
    ...
    Eigen::Matrix3d R_delta_double = R_delta.cast<double>();
    //std::cout << "============setMaxCorrespondenceDistance===================" << std::endl;
    //std::cout << R_delta_double << std::endl;

    // Set number of threads for multi-threading
    stable_gicp_speed->setNumThreads(1, R_delta_double); // Adjust number of threads as needed
  ...
  void main_callback(const sensor_msgs::PointCloud2ConstPtr &msg)
  {
	...
	*cloud_in_boxed_for_local = *cloud_in_filtered;
	...
	else
	{
		...
		Eigen::Vector3f angle_velocity_average, acceleration_average, mean_acc;
		Eigen::Matrix3f R_delta;
		Eigen::Vector3f T_delta;
		
		Eigen::Matrix3f R_delta_all_imu = Eigen::Matrix3f::Identity ();
		double dt = 0;
		...
		for (auto it_imu = imu_buffer.begin(); it_imu < (imu_buffer.end() - 1); it_imu++)
		{
			...
			angle_velocity_average << 	0.5 * (head->angular_velocity.x + tail->angular_velocity.x),
					0.5 * (head->angular_velocity.y + tail->angular_velocity.y),
					0.5 * (head->angular_velocity.z + tail->angular_velocity.z);
					
			acceleration_average << 	0.5 * (head->linear_acceleration.x + tail->linear_acceleration.x),
			...
			R_delta = Exp(angle_velocity_average, dt); // Assuming Exp() returns Eigen::Matrix3f
			
			R_delta_all_imu *= R_delta;
			
			acceleration_average(2) = 0;
			
			//cout<<"==================acceleration_average======================"<<endl;
			//cout<<angle_velocity_average<<endl;
			//cout<<acceleration_average<<endl;
			
			
			if( abs( angle_velocity_average(2) ) > 1 || abs( acceleration_average(1) ) > 3.0  )
			{
				continue;
			}
			...
			// Update velocity using acceleration (simple Euler integration for velocity)
			//velocity += R_delta * acceleration_average * dt;

			// Compute translation delta using velocity and acceleration
			//Eigen::Vector3f T_delta = velocity * dt + 0.5 * (R_delta * acceleration_average) * dt * dt;
			//Eigen::Vector3f T_delta = 0.5 * (R_delta * acceleration_average) * dt * dt;
			T_delta = 0.5 * (1 * acceleration_average) * dt * dt;
		...
		// the start point of LM algorithm, to the gradient zero point
		...
		#pragma omp simd
		for (int i = 0; i < num_threads; ++i) {
			//threads.emplace_back(runGICP, cloud_in_filtered, cloud_pre, std::ref(results[i]), max_correspondence_dists[i], max_iterations[i], initial_guesses[i], R_delta_all_imu);
			threads.emplace_back(runGICP, cloud_in_filtered, cloud_pre, std::ref(results[i]), max_correspondence_dists[i], max_iterations[i], initial_guesses[i], Eigen::Matrix3f::Identity());
		...
		// Assign the rotation part to the top-left 3x3 block
		//best_result.transformation.block<3, 3>(0, 0) = R_delta_all_imu;
