    ...
    : b_first_frame_(true), imu_need_init_(true), start_timestamp_(-1)
{
  //初始化迭代次数
  init_iter_num = 1;
  //调用use-ikfom.hpp里面的process_noise_cov完成噪声协方差的初始化
  Q = process_noise_cov();
  //加速度测量协方差初始化
...
void ImuProcess::IMU_init(const MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, int &N)
{
  ...
  //将最后一帧的imu数据传入last_imu_中，暂时没用到 
  last_imu_ = meas.imu.back();
}

void ImuProcess::UndistortPcl(const MeasureGroup &meas, 
					esekfom::esekf<state_ikfom, 
					12, 
					input_ikfom> &kf_state, 
					PointCloudXYZI &pcl_out)
{
  ...
  for (auto it_imu = v_imu.begin(); it_imu < (v_imu.end() - 1); it_imu++)
  {
    ...
    cout<<"=============gravity=============="<<endl;
    cout<<imu_state.rot<<endl;
    cout<<acc_avr<<endl;
    cout<<imu_state.ba<<endl;
    cout<<"=============gravity[1]=============="<<endl;
    cout<<imu_state.grav<<endl;
    ...
    if( abs(imu_state.vel[0]) > 1 || abs(imu_state.vel[1]) > 1 || abs(imu_state.vel[2]) > 1 )
    {
    	imu_state.vel << 0.0, 0.0, 0.0;
    }
    ...
    cout<<imu_state.vel<<endl;
    cout<<"=========[4_imu_position]=========="<<endl;
    cout<<imu_state.pos<<endl;
    cout<<"=========[5_imu_R]=========="<<endl;
    cout<<imu_state.rot.toRotationMatrix()<<endl;
