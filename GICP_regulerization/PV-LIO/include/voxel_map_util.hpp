                    ...
                    std::unordered_map<VOXEL_LOC, OctoTree *> &feat_map)
{
  ...
  for (uint i = 0; i < plsize; i++) 
  {
    ...
    if (i==0)
    {
      position_LZ.x=(int64_t)loc_xyz[0];
      position_LZ.y=(int64_t)loc_xyz[1];
      position_LZ.z=(int64_t)loc_xyz[2];
    }
    auto iter = feat_map.find(position);
    // 如果点的位置已经存在voxel 那么就更新点的位置 否则创建新的voxel
    if (iter != feat_map.end()) 
...
std::unordered_map<VOXEL_LOC, OctoTree *>::iterator it = feat_map.begin();
  while(it != feat_map.end())
  {
    ...
    if ((abs(loc.x-position_LZ.x)>100 || abs(loc.y-position_LZ.y)>100 || abs(loc.z-position_LZ.z)>100))
    {
      delete it->second;
      it->second =NULL;
      feat_map.erase(it++);
