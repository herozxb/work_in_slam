...
// publish feature_undistort  or  feature_down_body
...
// all the thing above is make the IMU time sync to lidar time
...
bool sync_packages(MeasureGroup &meas)
{
    ...
    if(!lidar_pushed)
    {
        ...
        else
        {
            ...
            //Checks if the time offset (in curvature field) of the last point in the scan is less than half the average scan time.
            ...
            //lidar_end_time is updated with the actual time of the last point in the scan
            lidar_end_time = meas.lidar_beg_time + meas.lidar->points.back().curvature / double(1000);
            
            //Updates lidar_mean_scantime by averaging in the time of this scan, providing a more accurate scan time estimate over multiple scans.
...
int main(int argc, char **argv) 
{
	...
	VD(DIM_STATE) solution; // DIM_STATE = 18, 1. rot_end  2. pos_end  3. vel_end 4. bias_g 5. bias_a  6. gravity
	MD(DIM_STATE, DIM_STATE) H_Transpose_multiply_R_inverse_multiply_H, I_STATE, G; // 7. cov
	...
	extra_R_parameter << extrinR[0], extrinR[1], extrinR[2], 
	...
	if (imu_en) {
		cout << "use imu" << std::endl;
	} else {
		cout << "no imu" << std::endl;
	}

	cout << "=============================max_voxel_size=============================" << max_voxel_size << std::endl;


	// gyroscope_cov_scale = 0.1
	p_imu->set_gyr_cov_scale(V3D(gyroscope_cov_scale, gyroscope_cov_scale, gyroscope_cov_scale));
	// accelerater_cov_scale = 0.1
	...
	while (status) 
	{
		...
		if (sync_packages(Measures)) 
		{
			...
			// init the first frame, and first map
			if (flag_EKF_initlized && !init_map) 
			{
				...
				for (size_t i = 0; i < world_lidar->size(); i++) 
				{
					...
					V3D point_body( feature_undistort->points[i].x, feature_undistort->points[i].y, feature_undistort->points[i].z);
					...
					// cov = cov_lidar
					M3D cov_lidar;  
					calcBodyCov(point_body, ranging_cov, angle_cov, cov_lidar);


					point_body += Lidar_offset_to_IMU;
					...
					// 2. world covarience of point
					M3D cov_world = state.rot_end * cov_lidar * state.rot_end.transpose() + (-point_crossmat) * state.cov.block<3, 3>(0, 0) * (-point_crossmat).transpose() + state.cov.block<3, 3>(3, 3);
				...
				// 1. pv_list, world point with cov,  2. max_voxel_size voxel size = 1.0, 3. max_layer = 2, 4. layer_size = [5, 5, 5, 5, 5], 5. max_points_size = 100, 6. max_points_size = 100, 7. min_eigen_value = 0.01, 8. voxel_map
			...
			sor_input.setInputCloud (feature_undistort);
			sor_input.setLeafSize (0.3f, 0.3f, 0.3f);
			sor_input.filter (*feature_down_body);
			...
			for (size_t i = 0; i < feature_down_body->size(); i++) 
			{
				...
				V3D point_body(feature_down_body->points[i].x, feature_down_body->points[i].y, feature_down_body->points[i].z);
				...
				{       
					...
					// point_to_plane_list is the key, Residual in it
					...
					MatrixXd R( effect_feature_number, effect_feature_number );
					...
					for (int i = 0; i < effect_feature_number; i++) 
					{
						...
						V3D point_body(laser_point.x, laser_point.y, laser_point.z);
						...
						V3D point_world = state.rot_end * point_body + state.pos_end;
						...
						J_nq.block<1, 3>(0, 0) = point_world - point_to_plane_list[i].center;   	//	(Tk * L_p(i) - q(i) ) ^ T
						J_nq.block<1, 3>(0, 3) = -point_to_plane_list[i].normal;			//	- n(i)^T	

						//J_L_p(i)<1, 3>(0, 0)  = (point_to_plane_list[i].normal)^T
						...
						/*** calculate the Measuremnt Jacobian matrix H ***/				// G(i) = u(i)^T
						V3D A(point_crossmat * state.rot_end.transpose() * norm_vector); 		// d( H - H ) / dx, dR/dx = -1 * G(i) * G_R_I * cross_mat( P_imu )
						Hsub.row(i) << VEC_FROM_ARRAY(A), norm_point.x, norm_point.y, norm_point.z;	// d( H - H ) / dx, dt/dx = G(i) 

						Hsub_Transpose_multiply_R_inverse.col(i) << A[0] * R_inverse(i), A[1] * R_inverse(i), A[2] * R_inverse(i), norm_point.x * R_inverse(i), norm_point.y * R_inverse(i), norm_point.z * R_inverse(i);
						/*** Measuremnt: distance to the closest surface/corner ***/
						measurement_vector(i) = -norm_point.intensity;

						//for(int k = 0; k < effect_feature_number; k++ )
						//{
						//    R(i,k) = 0;
						//    R(i,i) = 1 / R_inverse(i);
						//}
					...
					if (!flag_EKF_initlized) 
					{
						...
						// [DONE]
						auto &&K_init = state.cov * H_init_Transpose * ( H_init * state.cov * H_init_Transpose + 0.0001 * MD(9, 9)::Identity() ).inverse();
					...
					else 
					{
						...
						// [DONE]
						// Q^-1 =  ( H^T * R^-1 * H + J^-T * P^-1 * J^-1 ) ^-1
						// DIM_STATE=18
						
						MD(DIM_STATE, DIM_STATE) &&Q_inverse = ( 1.0 * H_Transpose_multiply_R_inverse_multiply_H + 1.0 * (state.cov).inverse() + 0.00001 * Hsub_Transpose * Hsub ).inverse(); 
						...
						////cout<<"=============Hsub * Hsub_Transpose.inverse()============="<<endl;
						////cout<< Hsub * Hsub_Transpose.inverse() <<endl;
						...
						//K = state.cov.block<DIM_STATE, 6>(0, 0) * Hsub_Transpose * ( Hsub * state.cov.block<6, 6>(0, 0) * Hsub_Transpose + 0.0001 * MD(effect_feature_number, effect_feature_number)::Identity() ).inverse();
						//K = state.cov.block<DIM_STATE, 6>(0, 0) * Hsub_Transpose * ( Hsub * state.cov.block<6, 6>(0, 0) * Hsub_Transpose +  1 * ( ( Hsub * Hsub_Transpose).inverse() * Hsub * Hsub_Transpose_multiply_R_inverse ).inverse()  ).inverse();
						//K = state.cov.block<DIM_STATE, 6>(0, 0) * Hsub_Transpose * ( Hsub * state.cov.block<6, 6>(0, 0) * Hsub_Transpose +  R ).inverse();
						...
						// dx = -J * box_minus + K * ( r + H * J * box_minus )
